...
class TicTacToe
	# Initialize an instance of this game
	def initialize
		initialize_game
	end
	...
	def display_piece_by_location(piece,location)
		if location == 2 || location == 5
			...
			print_piece(piece, SEPARATOR[:verticle])
		end
	end
	...
	# Set location speciied by move to the current player's piece
	def set_move move
		@board[move] = @player
	end
	...
	def get_move
		while true
			question = "Player #{@player},m Enter play position (1-9)"
	...
	# Get the repnse to a question from the player
	...
	def game_over?
		win_formations = [[0,1,2],[3,4,5],[6,7,8,],[0,4,8],[2,4,6],[0,3,6],[1,4,7],[2,5,7]]
	...
	# Determine if a player has pieces in a certain foramtion
	...
	PIECE = {x: 'X', o: 'O', blank: ' '}
	SEPARATOR = {verticle: "\u2503", horizontal: "\u2501", cross: "\u254b"}
	HOW_TO_PLAY_TEXT = "
	|
	| 1 | 2 | 3
	|---+---+--
